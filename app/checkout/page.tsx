...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    if (
      ...
      if (!isValidNameOrLastname(checkoutForm.name)) {
        toast.error("Anda memasukkan format yang salah untuk nama");
        return;
      }

      if (!isValidNameOrLastname(checkoutForm.lastname)) {
        toast.error("Anda memasukkan format yang salah untuk nama belakang");
        return;
      }

      if (!isValidEmailAddressFormat(checkoutForm.email)) {
        toast.error("Anda memasukkan format yang salah untuk alamat email");
        return;
      }

      if (!isValidNameOrLastname(checkoutForm.cardName)) {
        toast.error("Anda memasukkan format yang salah untuk nama kartu");
        return;
      }

      if (!isValidCardNumber(checkoutForm.cardNumber)) {
        toast.error("Anda memasukkan format yang salah untuk nomor kartu kredit");
        return;
      }

      if (!isValidCreditCardExpirationDate(checkoutForm.expirationDate)) {
        toast.error(
          "Anda memasukkan format yang salah untuk tanggal kedaluwarsa kartu kredit"
        );
        return;
      }

      if (!isValidCreditCardCVVOrCVC(checkoutForm.cvc)) {
        toast.error("Anda memasukkan format yang salah untuk CVC atau CVV kartu kredit");
        return;
      }
      ...
      toast.error("Anda perlu memasukkan nilai di bidang input");
    }
  };
  ...
  useEffect(() => {
    if (products.length === 0) {
      toast.error("Anda tidak memiliki barang di keranjang Anda");
      router.push("/cart");
    }
  }, []);
          ...
          <div className="mx-auto max-w-lg lg:max-w-none">
            ...
            <div className="mt-10 border-t border-gray-200 pt-6 ml-0">
              <button
                ...
                className="w-full rounded-md border border-transparent bg-rose-500 px-20 py-2 text-lg font-medium text-white shadow-sm hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-gray-50 sm:order-last"
              >
                Pay Now
              </button>
            </div>
