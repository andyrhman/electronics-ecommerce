...
const WishItem = ({
    ...
    if (userId) {
      ...
      fetch(`http://localhost:3001/api/wishlist/${userId}/${productId}`, {method: "DELETE"}).then(
        (response) => {
          ...
          toast.success("Item dihapus dari daftar wishlist Anda");
        }
      );
    }else{
  ...
  return (
    <tr className="hover:bg-gray-100 cursor-pointer">
      ...
      >
        {stockAvailabillity ? (
          <span className="text-success">Stok tersedia</span>
        ) : (
          <span className="text-error">Stok habis</span>
        )}
      </td>
      <td>
        <button className="btn btn-xs bg-rose-500 text-white hover:text-rose-500 border border-blue-500 hover:bg-white hover:text-rose-500 text-sm">
          <FaHeartCrack />
          <span
            className="max-sm:hidden"
            onClick={() => deleteItemFromWishlist(id)}
          >
            hapus dari daftar wishlist
          </span>
        </button>
      </td>
    </tr>
