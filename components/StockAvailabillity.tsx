...
const StockAvailabillity = ({ stock, inStock } : { stock: number, inStock: number }) => {
  return (
    <p className='text-xl flex gap-x-2 max-[500px]:justify-center'>Ketersediaan: 
    { inStock === 1 ? <span className='text-success flex items-center gap-x-1'>Stok tersedia <FaCheck /></span> :  <span className='text-error flex items-center gap-x-1'>Kehabisan stok <FaXmark /></span>}
